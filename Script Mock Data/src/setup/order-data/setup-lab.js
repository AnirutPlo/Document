...
export async function setupLab(logger, _skipRecordsTracker, dataDir, { cortexApiClient } = {}) {
  if (!cortexApiClient) {
    throw new Error('cortexApiClient is required');
  }

  const labDir = join(dataDir, 'order-data/lab');
  ...
  for (const fileName of files) {
    ...
    try {
      ...
      const result = await cortexApiClient.importCsv('/labs/import', filePath);
